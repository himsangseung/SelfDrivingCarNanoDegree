...
FusionEKF::FusionEKF() {
  ...
  // initialize state measurement matrix
  ekf_.x_ = VectorXd(4);
  ekf_.x_ << 1, 1, 1, 1;
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      // Placeholder below for alternative init - not used 
      // ekf_.Init( ekf_.x_, P_, F_, Hj_, R_, Q_ );
  ...
  */
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    // Radar updates
    ekf_.R_ = R_radar_; 
    ekf_.H_ = toolsJacobian.CalculateJacobian(ekf_.x_);
