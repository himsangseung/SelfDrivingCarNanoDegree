...
class KalmanFilter {
 public:
  ...
  /**
   * Preprocess the state by using standard Kalman Filter equations
   * @param z The measurement at k+1
   */
  void Update(const Eigen::VectorXd &z);

  /**
   * Preprocess the state by using Extended Kalman Filter equations
   * @param z The measurement at k+1
   */
  void UpdateEKF(const Eigen::VectorXd &z);

  /**
   * Actual process update of the state by using Kalman Filter equations
